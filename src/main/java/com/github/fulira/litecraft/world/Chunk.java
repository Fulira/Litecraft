...
public class Chunk implements BlockAccess, WorldGenConstants, SODSerializable {
	 ...
	 * @return creates a long that represents a coordinate, for use as a key in
	 *         arrays.
	 */
	 ...
	 * long and must only be read linearly.
	 */
				...
				for (int x = 0; x < CHUNK_SIZE; x++)
					for (int y = 0; y < CHUNK_SIZE; y++)
						for (int z = 0; z < CHUNK_SIZE; z++) {
							...
							// Check for chunk edges to avoid errors when get the neighboring blocks, TODO
							// fix this
							...
							try {
								if (getBlockInstance(x - 1, y, z).getModel() == null
										|| getBlockInstance(x + 1, y, z).getModel() == null
										|| getBlockInstance(x, y - 1, z).getModel() == null
										|| getBlockInstance(x, y + 1, z).getModel() == null
										|| getBlockInstance(x, y, z - 1).getModel() == null
										|| getBlockInstance(x, y, z + 1).getModel() == null) {
									tempList.add(block);
								}
	...
	public void read(BinaryData data) {
		...
		for (int z = 0; z < CHUNK_SIZE; ++z) // z, y, x order for data saving and loading so we can use incremental pos
												// hashes
			for (int y = 0; y < CHUNK_SIZE; ++y)
				for (int x = 0; x < CHUNK_SIZE; ++x) {
		...
		try {
			...
			if (!readExceptionNotif) {
				System.out.println(
						"An exception occurred reading properties for a chunk! This could be a benign error due to updates to chunk properties.");
