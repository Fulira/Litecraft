...
public final class LitecraftSave {
	public LitecraftSave(String name, boolean mustCreateNew) {
		...
		if (mustCreateNew) {
			while (saveDir.exists()) {
				sb.append('_'); // append "_" to the save name until we get a unique save, if we must create a
								// new save
	...
	public boolean saveChunk(Chunk chunk) {
		StringBuilder fileLocBuilder = new StringBuilder(this.file.getPath()).append('/')
				.append(Dimension.getById(chunk.dimension).saveIdentifier).append('/').append(chunk.chunkX).append('/')
				.append(chunk.chunkZ);
	...
	public Chunk readChunk(World world, int chunkX, int chunkY, int chunkZ, int dimension) {
		// format: <save dir>/<dim>/<chunkX>/<chunkZ>/<chunkY>.sod
		File chunkFile = new File(new StringBuilder(this.file.getPath()).append('/')
				.append(Dimension.getById(dimension).saveIdentifier).append('/').append(chunkX).append('/')
				.append(chunkZ).append('/').append(chunkY).append(".sod").toString());
		if (chunkFile.isFile()) {
	...
	public World getWorldOrCreate(Dimension<?> dim) {
		...
		if (globalDataFile.isFile()) // load world
		{
			...
			DataSection properties = data.get("properties"); // get the properties data section from the data that we
																// have just read
			...
			try // try read the seed from the file
			{
				...
				System.out.println(
						"Exception in reading save data! This may be benign, merely an artifact of an update to the contents of the world save data.");
			}
			World world = new World(seed, RENDER_SIZE, dim, this); // create new world with seed read from file or 0, if
		...
		} else // create world
		{
			...
			try {
				...
				this.writeGlobalData(globalDataFile, seed, new Vector3f(0, 0, -3)); // write global data with default
																					// player pos
			} catch (IOException e) {
